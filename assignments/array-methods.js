...
let allCaps = runners.map(item => `${item.first_name.toUpperCase()} ${item.last_name}`);
console.log(allCaps); 
...
let ticketPriceTotal = runners.map(item => item.donation).reduce((acc, curr) => {
  return acc += curr;
}, 0);
...
// Problem 1: Find the highest amount of individual donation. 
let highestDonation = runners.reduce((acc, curr) => {
  return Math.max(acc, curr.donation);
}, runners[0].donation);
console.log(highestDonation);

// Problem 2: Find the company that has donated the most money.
let topDonor = runners.filter(item => item.donation === highestDonation)[0].company_name;
console.log(topDonor);

// Problem 3: Find how many S and M shirts are needed.
let numSandM = runners.filter(item => item.shirt_size === "S" || item.shirt_size === "M").length;
console.log(numSandM);
